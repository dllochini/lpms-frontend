...
export default function ProgressTrack() {
  ...
  const handleApprovalChange = (taskId, value) => {
    setTasks((prev) =>
      prev.map((t) => (t.id === taskId ? { ...t, approval: value } : t))
    );
  };
            ...
            <Paper key={task.id} sx={{ p: 2, borderRadius: 2 }} elevation={1}>
              ...
              <Collapse in={expandedTaskId === task.id} timeout="auto" unmountOnExit>
                <Box sx={{ mt: 2 }}>
                  ...
                  {/* Buttons + Approval under the table */}
                  <Box
                    sx={{
                      display: "flex",
                      justifyContent: "space-between",
